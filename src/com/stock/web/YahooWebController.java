...
public class YahooWebController {
	...
	public void updateRecentQuote() {
		...
		List<CompanyModel> tsxCompanies = manager
				.findCompanyByExchange("Toronto");
	...
	public String upload(@RequestParam String action, Model model)
			throws IOException {
		if (action.equals("updateExchange")) {
			...
			for (int i = 0; i < companyList.size(); i++) {
				StockInfoModel stockModel = yahooRequest
						.getRecentQuote(companyList.get(i).getSymbol());
			...
			hr = yahooRequest.getHistoricalQuoteCsv(symbol, "01", "01", "2000",
					"31", "12", "2014");
