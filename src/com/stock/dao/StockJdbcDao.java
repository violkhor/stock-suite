...
public class StockJdbcDao implements StockDao {
	...
	public void insert(final List<CompanyModel> companyList) {
		...
		for (int j = 0; j < companyList.size(); j += batchSize) {
			...
			final List<CompanyModel> batchList = companyList.subList(j, j
					+ batchSize > companyList.size() ? companyList.size() : j
					+ batchSize);

			jdbcTemplate.batchUpdate(INSERT_COMPANY,
			...
			jdbcTemplate.batchUpdate(INSERT_HISTORICAL,
					new BatchPreparedStatementSetter() {
						...
						public void setValues(PreparedStatement ps, int i)
							...
							ps.setDate(2, new java.sql.Date(hr.getTradeDate()
									.getTime()));
	...
	public void updateCompany(List<CompanyModel> companyList) {
		...
		for (int j = 0; j < companyList.size(); j += batchSize) {
			...
			jdbcTemplate.batchUpdate(UPDATE_COMPANY,
					new BatchPreparedStatementSetter() {
						@Override
						public void setValues(PreparedStatement ps, int i)
								throws SQLException {
	...
	public List<CompanyModel> findAllCompanies() {
		...
		return jdbcTemplate.query(SELECT_COMPANY_ALL,
				new CompanyModelRowMapper());
	}
	...
	public void insertRecentStockInfo(List<StockInfoModel> stockList) {
		...
		for (int j = 0; j < stockList.size(); j += batchSize) {
			...
			final List<StockInfoModel> batchList = stockList.subList(j, j
					+ batchSize > stockList.size() ? stockList.size() : j
					+ batchSize);

			jdbcTemplate.batchUpdate(INSERT_RECENT,
					new BatchPreparedStatementSetter() {
						@Override
						public void setValues(PreparedStatement ps, int i)
								throws SQLException {
							...
							ps.setString(16,
									model.getChangeFromYearLowPercent());
							ps.setString(17,
									model.getChangeFromYearHighPercent());
	...
	public List<StockInfoModel> getAllRecentStockInfo() {
		return jdbcTemplate.query(SELECT_STOCK_INFO_ALL,
				new StockInfoRowMapper());
	}

	@Override
	public void insertOrderDetails(final List<Orders> orders) {
		jdbcTemplate.batchUpdate(INSERT_ORDERS,
				new BatchPreparedStatementSetter() {
					...
					public void setValues(PreparedStatement ps, int i)
						...
						ps.setDate(1, new java.sql.Date(order.getOrderDate()
								.getTime()));
